function [Vega]=BSeuCallVegaII_UniformGrid(S,K,T,r,sig)
...
n_price_steps_per_strike = 100000; 
Smax_multiplier = 1.2;
stick_strike_to_node = 1;

%settings for time grid
n_time_steps_per_year = 6000;
min_time_steps_per_interval = 1500;
...
sig2 = 1.00001 * sig;
[S_FDM2 U_FDM2 payoff2] = FDMUniformGrid( is_call, is_American_style, ...
    T, K, barrier_type, B, ...
    r, q, volatility_model, sig2, ...
    div_datetimes, div_amounts, div_percentages, ... 
    n_price_steps_per_strike, Smax_multiplier, stick_strike_to_node, ...
    n_time_steps_per_year, min_time_steps_per_interval, n_Rannacher_sub_steps);

Vega_FDM = (U_FDM2 - U_FDM) / (sig2 - sig);

Vega = InterpolateFromUniformGrid(S, S_FDM, Vega_FDM);
