function [Vega]=BSeuCallVegaII_COS(S,K,T,r,sig)
% BENCHOP Problem 1: The Black-Scholes-Merton model for one underlying asset
% BSeuCallVegaII_COS computes the Vega for a European call option (Challenging parameters)
%
...
Recf_Vega = repmat(exp(-0.5*sig^2*T*omega.^2).*(sig*omega*T),1,length(x)).*sin(omega*((r-0.5*sig^2)*T+x-a))+...
            repmat(exp(-0.5*sig^2*T*omega.^2).*(-sig*omega.^2*T),1,length(x)).*cos(omega*((r-0.5*sig^2)*T+x-a));

% Option's Vega
Vega = exp(-r*T)*2/(b-a)*K*(Uk*Recf_Vega);
